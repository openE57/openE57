...
struct GroupBuffers
{
  ...
  GroupBuffers()
  : idElementValue(NULL), startPointIndex(NULL), pointCount(NULL), cartesianBoundsXMinimum(NULL), cartesianBoundsXMaximum(NULL), cartesianBoundsYMinimum(NULL),
    cartesianBoundsYMaximum(NULL), cartesianBoundsZMinimum(NULL), cartesianBoundsZMaximum(NULL), sphericalBoundsRangeMinimum(NULL),
    sphericalBoundsRangeMaximum(NULL), sphericalBoundsAzimuthStart(NULL), sphericalBoundsAzimuthEnd(NULL), sphericalBoundsElevationMinimum(NULL),
  ...
  ~GroupBuffers()
  {
    ...
    if (pointCount)
      delete[] pointCount;
    if (cartesianBoundsXMinimum)
      delete[] cartesianBoundsXMinimum;
    if (cartesianBoundsXMaximum)
      delete[] cartesianBoundsXMaximum;
    if (cartesianBoundsYMinimum)
      delete[] cartesianBoundsYMinimum;
    if (cartesianBoundsYMaximum)
      delete[] cartesianBoundsYMaximum;
    if (cartesianBoundsZMinimum)
      delete[] cartesianBoundsZMinimum;
    if (cartesianBoundsZMaximum)
      delete[] cartesianBoundsZMaximum;
    if (sphericalBoundsRangeMinimum)
      delete[] sphericalBoundsRangeMinimum;
    if (sphericalBoundsRangeMaximum)
      delete[] sphericalBoundsRangeMaximum;
    if (sphericalBoundsAzimuthStart)
      delete[] sphericalBoundsAzimuthStart;
    if (sphericalBoundsAzimuthEnd)
      delete[] sphericalBoundsAzimuthEnd;
    if (sphericalBoundsElevationMinimum)
      delete[] sphericalBoundsElevationMinimum;
    if (sphericalBoundsElevationMaximum)
      delete[] sphericalBoundsElevationMaximum;
  };
};
...
struct PointBuffers
{
  ...
  PointBuffers()
  : cartesianX(NULL), cartesianY(NULL), cartesianZ(NULL), sphericalRange(NULL), sphericalAzimuth(NULL), sphericalElevation(NULL), rowIndex(NULL),
    columnIndex(NULL), returnCount(NULL), returnIndex(NULL), timeStamp(NULL), intensity(NULL), colorRed(NULL), colorGreen(NULL), colorBlue(NULL),
    cartesianInvalidState(NULL), sphericalInvalidState(NULL), isTimeStampInvalid(NULL), isIntensityInvalid(NULL), isColorInvalid(NULL){};
  ...
  vector<SourceDestBuffer> allBuffers(ImageFile imf)
  {
    ...
    if (cartesianX)
      dbufs.push_back(SourceDestBuffer(imf, "cartesianX", cartesianX, elementCount, true, true));
    if (cartesianY)
      dbufs.push_back(SourceDestBuffer(imf, "cartesianY", cartesianY, elementCount, true, true));
    if (cartesianZ)
      dbufs.push_back(SourceDestBuffer(imf, "cartesianZ", cartesianZ, elementCount, true, true));
    if (sphericalRange)
      dbufs.push_back(SourceDestBuffer(imf, "sphericalRange", sphericalRange, elementCount, true, true));
    if (sphericalAzimuth)
      dbufs.push_back(SourceDestBuffer(imf, "sphericalAzimuth", sphericalAzimuth, elementCount, true, true));
    if (sphericalElevation)
      dbufs.push_back(SourceDestBuffer(imf, "sphericalElevation", sphericalElevation, elementCount, true, true));
    if (rowIndex)
      dbufs.push_back(SourceDestBuffer(imf, "rowIndex", rowIndex, elementCount, true, true));
    if (columnIndex)
      dbufs.push_back(SourceDestBuffer(imf, "columnIndex", columnIndex, elementCount, true, true));
    if (returnCount)
      dbufs.push_back(SourceDestBuffer(imf, "returnCount", returnCount, elementCount, true, true));
    if (returnIndex)
      dbufs.push_back(SourceDestBuffer(imf, "returnIndex", returnIndex, elementCount, true, true));
    if (timeStamp)
      dbufs.push_back(SourceDestBuffer(imf, "timeStamp", timeStamp, elementCount, true, true));
    if (intensity)
      dbufs.push_back(SourceDestBuffer(imf, "intensity", intensity, elementCount, true, true));
    if (colorRed)
      dbufs.push_back(SourceDestBuffer(imf, "colorRed", colorRed, elementCount, true, true));
    if (colorGreen)
      dbufs.push_back(SourceDestBuffer(imf, "colorGreen", colorGreen, elementCount, true, true));
    if (colorBlue)
      dbufs.push_back(SourceDestBuffer(imf, "colorBlue", colorBlue, elementCount, true, true));
    if (cartesianInvalidState)
      dbufs.push_back(SourceDestBuffer(imf, "cartesianInvalidState", cartesianInvalidState, elementCount, true, true));
    if (sphericalInvalidState)
      dbufs.push_back(SourceDestBuffer(imf, "sphericalInvalidState", sphericalInvalidState, elementCount, true, true));
    if (isTimeStampInvalid)
      dbufs.push_back(SourceDestBuffer(imf, "isTimeStampInvalid", isTimeStampInvalid, elementCount, true, true));
    if (isIntensityInvalid)
      dbufs.push_back(SourceDestBuffer(imf, "isIntensityInvalid", isIntensityInvalid, elementCount, true, true));
    if (isColorInvalid)
      dbufs.push_back(SourceDestBuffer(imf, "isColorInvalid", isColorInvalid, elementCount, true, true));
    return (dbufs);
  };
...
LineGroup::LineGroup()
: lineIndex(0), declaredStartDefined(false), declaredCountDefined(false), declaredCartesianBoundsDefined(false), declaredSphericalBoundsDefined(false),
  declaredStart(0), declaredCount(0), actualStart(0), actualCount(0)
{}
  ...
  do
  {
    ...
    for (size_t i = 0; i < gotCount; i++)
    {
      ...
      if (gbufs.cartesianBoundsXMinimum)
      {
        ...
        lineGroup.declaredCartesianBounds = BoundingBox(gbufs.cartesianBoundsXMinimum[i], gbufs.cartesianBoundsXMaximum[i], gbufs.cartesianBoundsYMinimum[i],
                                                        gbufs.cartesianBoundsYMaximum[i], gbufs.cartesianBoundsZMinimum[i], gbufs.cartesianBoundsZMaximum[i]);
      }
      if (gbufs.sphericalBoundsRangeMinimum)
      {
        lineGroup.declaredSphericalBoundsDefined = true;
        lineGroup.declaredSphericalBounds
          = BoundingBox(gbufs.sphericalBoundsRangeMinimum[i], gbufs.sphericalBoundsRangeMaximum[i], gbufs.sphericalBoundsAzimuthStart[i],
                        gbufs.sphericalBoundsAzimuthEnd[i], gbufs.sphericalBoundsElevationMinimum[i], gbufs.sphericalBoundsElevationMaximum[i]);
      }
...
E57Validator::E57Validator(E57ValidatorOptions opts, std::ostream& os) : options_(opts), os_(os)
{
...
void E57Validator::validateData3D(Node n)
{
  ...
  for (int64_t i = 0; i < v.childCount(); i++)
  {
    ...
    for (int64_t j = 0; j < sn.childCount(); j++)
    {
      ...
      else if (childName == "intensityLimits")
      { // added by sc
        validateIntensityLimits(childn);
      }
      else if (childName == "colorLimits")
      { // added by sc
...
void E57Validator::validatePointRecordContents(CompressedVectorNode cv, StructureNode proto, PointBuffers& pbufs)
{
  ...
  do
  {
    ...
    for (size_t i = 0; i < gotCount; i++)
    {
      ...
      {
        if (!pbufs.cartesianInvalidState || pbufs.cartesianInvalidState[i] == 0)
        { /// This point coordinate is valid
          /// Calc spherical coords from cartesian using formulas from 5.5 of ASTM E57 standard
          double range
            = sqrt(pbufs.cartesianX[i] * pbufs.cartesianX[i] + pbufs.cartesianY[i] * pbufs.cartesianY[i] + pbufs.cartesianZ[i] * pbufs.cartesianZ[i]);
          double azimuth = 0.0;
          ...
          if (cartesianBoundsDefined && cartesianBounds.notEmpty)
          {
            if ((pbufs.cartesianX[i] < cartesianBounds.minimum[0]) || (cartesianBounds.maximum[0] < pbufs.cartesianX[i]))
              PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"), "value " + toString(pbufs.cartesianX[i]) + " is out of Cartesian bounds", &cv,
                            blockStart + i);
            if ((pbufs.cartesianY[i] < cartesianBounds.minimum[1]) || (cartesianBounds.maximum[1] < pbufs.cartesianY[i]))
              PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianY"), "value " + toString(pbufs.cartesianY[i]) + " is out of Cartesian bounds", &cv,
                            blockStart + i);
            if ((pbufs.cartesianZ[i] < cartesianBounds.minimum[2]) || (cartesianBounds.maximum[2] < pbufs.cartesianZ[i]))
              PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianZ"), "value " + toString(pbufs.cartesianZ[i]) + " is out of Cartesian bounds", &cv,
                            blockStart + i);
          }
          ...
          if (sphericalBoundsDefined && sphericalBounds.notEmpty)
          {
            ...
            if ((azimuth < sphericalBounds.minimum[1]) || (sphericalBounds.maximum[1] < azimuth))
              PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"), "converted azimuth " + toString(azimuth) + " is out of spherical bounds", &cv,
                            blockStart + i);
            if ((elevation < sphericalBounds.minimum[2]) || (sphericalBounds.maximum[2] < elevation))
              PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"), "converted elevation " + toString(elevation) + " is out of spherical bounds", &cv,
                            blockStart + i);
          }
          ...
          if (usesLineGroups)
          {
            ...
            if (lineGroup)
            {
              ...
              if (lineGroup->declaredCartesianBoundsDefined)
              {
                ...
                if (pbufs.cartesianX[i] < lineGroup->declaredCartesianBounds.minimum[0] || lineGroup->declaredCartesianBounds.maximum[0] < pbufs.cartesianX[i])
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"),
                                "value " + toString(pbufs.cartesianX[i]) + " is out of Cartesian bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
                if (pbufs.cartesianY[i] < lineGroup->declaredCartesianBounds.minimum[1] || lineGroup->declaredCartesianBounds.maximum[1] < pbufs.cartesianY[i])
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianY"),
                                "value " + toString(pbufs.cartesianY[i]) + " is out of Cartesian bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
                if (pbufs.cartesianZ[i] < lineGroup->declaredCartesianBounds.minimum[2] || lineGroup->declaredCartesianBounds.maximum[2] < pbufs.cartesianZ[i])
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianZ"),
                                "value " + toString(pbufs.cartesianZ[i]) + " is out of Cartesian bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
              }
              ...
              {
                if (range < lineGroup->declaredSphericalBounds.minimum[0] || lineGroup->declaredSphericalBounds.maximum[0] < range)
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"),
                                "converted range " + toString(range) + " is out of Spherical bounds of line group " + toString(lineIndex), &cv, blockStart + i);
                }
                if (azimuth < lineGroup->declaredSphericalBounds.minimum[1] || lineGroup->declaredSphericalBounds.maximum[1] < azimuth)
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"),
                                "converted azimuth " + toString(azimuth) + " is out of Spherical bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
                if (elevation < lineGroup->declaredSphericalBounds.minimum[2] || lineGroup->declaredSphericalBounds.maximum[2] < elevation)
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("cartesianX"),
                                "converted elevation " + toString(elevation) + " is out of Spherical bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
              }
      ...
      {
        if (!pbufs.sphericalInvalidState || pbufs.sphericalInvalidState[i] == 0)
        { /// This point coordinate is valid
          ...
          if (sphericalBoundsDefined && sphericalBounds.notEmpty)
          {
            if (pbufs.sphericalRange[i] < sphericalBounds.minimum[0] || sphericalBounds.maximum[0] < pbufs.sphericalRange[i])
              PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalRange"), "value " + toString(pbufs.sphericalRange[i]) + " is out of spherical bounds", &cv,
                            blockStart + i);
            if (pbufs.sphericalAzimuth[i] < sphericalBounds.minimum[1] || sphericalBounds.maximum[1] < pbufs.sphericalAzimuth[i])
              PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalAzimuth"), "value " + toString(pbufs.sphericalAzimuth[i]) + " is out of spherical bounds", &cv,
                            blockStart + i);
            if (pbufs.sphericalElevation[i] < sphericalBounds.minimum[2] || sphericalBounds.maximum[2] < pbufs.sphericalElevation[i])
              PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalElevation"), "value " + toString(pbufs.sphericalElevation[i]) + " is out of spherical bounds",
                            &cv, blockStart + i);
          }
          ...
          if (usesLineGroups)
          {
            ...
            if (lineGroup)
            {
              ...
              if (lineGroup->declaredSphericalBoundsDefined)
              {
                    ...
                    || lineGroup->declaredSphericalBounds.maximum[0] < pbufs.sphericalRange[i])
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalRange"),
                                "value " + toString(pbufs.sphericalRange[i]) + " is out of Spherical bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
                    ...
                    || lineGroup->declaredSphericalBounds.maximum[1] < pbufs.sphericalAzimuth[i])
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalAzimuth"),
                                "value " + toString(pbufs.sphericalAzimuth[i]) + " is out of Spherical bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
                    ...
                    || lineGroup->declaredSphericalBounds.maximum[2] < pbufs.sphericalElevation[i])
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalElevation"),
                                "value " + toString(pbufs.sphericalElevation[i]) + " is out of Spherical bounds of line group " + toString(lineIndex), &cv,
                                blockStart + i);
                }
              }
              ...
              if (lineGroup->declaredCartesianBoundsDefined)
              {
                if (x < lineGroup->declaredCartesianBounds.minimum[0] || lineGroup->declaredCartesianBounds.maximum[0] < x)
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalRange"),
                                "converted x " + toString(x) + " is out of Cartesian bounds of line group " + toString(lineIndex), &cv, blockStart + i);
                }
                if (y < lineGroup->declaredCartesianBounds.minimum[1] || lineGroup->declaredCartesianBounds.maximum[1] < y)
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalRange"),
                                "converted y " + toString(y) + " is out of Cartesian bounds of line group " + toString(lineIndex), &cv, blockStart + i);
                }
                if (z < lineGroup->declaredCartesianBounds.minimum[2] || lineGroup->declaredCartesianBounds.maximum[2] < z)
                {
                  PRINT_MESSAGE(1000 /*???*/, proto.get("sphericalRange"),
                                "converted z " + toString(z) + " is out of Cartesian bounds of line group " + toString(lineIndex), &cv, blockStart + i);
                }
              }
      ...
      if (pbufs.rowIndex)
      {
        ...
        if (pbufs.rowIndex[i] < indexBounds.minimum[0] || indexBounds.maximum[0] < pbufs.rowIndex[i])
        {
          PRINT_MESSAGE(1000 /*???*/, proto.get("rowIndex"),
                        "value " + toString(pbufs.rowIndex[i]) + " is out of indexBounds [" + toString(indexBounds.minimum[0]) + ","
                          + toString(indexBounds.maximum[0]) + "]",
                        &cv, blockStart + i);
        }
      ...
      if (pbufs.columnIndex)
      {
        ...
        if (pbufs.columnIndex[i] < indexBounds.minimum[1] || indexBounds.maximum[1] < pbufs.columnIndex[i])
        {
          PRINT_MESSAGE(1000 /*???*/, proto.get("columnIndex"),
                        "value " + toString(pbufs.columnIndex[i]) + " is out of indexBounds [" + toString(indexBounds.minimum[1]) + ","
                          + toString(indexBounds.maximum[1]) + "]",
                        &cv, blockStart + i);
        }
      ...
      if (pbufs.returnIndex && pbufs.returnCount)
      {
        ...
        {
          PRINT_MESSAGE(1000 /*???*/, proto.get("returnIndex"), "value " + toString(pbufs.returnIndex[i]) + " is negative", &cv, blockStart + i);
        }
        if (pbufs.returnCount[i] <= pbufs.returnIndex[i])
        {
          PRINT_MESSAGE(1000 /*???*/, proto.get("returnIndex"),
                        "value " + toString(pbufs.returnIndex[i]) + " is >= returnCount " + toString(pbufs.returnCount[i]), &cv, blockStart + i);
        }
        ...
        {
          PRINT_MESSAGE(1000 /*???*/, proto.get("returnIndex"),
                        "value " + toString(pbufs.returnIndex[i]) + " is out of indexBounds [" + toString(indexBounds.minimum[2]) + ","
                          + toString(indexBounds.maximum[2]) + "]",
                        &cv, blockStart + i);
        }
        ...
        if (blockStart + i == 0 || prevReturnIndex == prevReturnCount - 1)
        {
          ...
          {
            PRINT_MESSAGE(1000 /*???*/, proto.get("returnIndex"),
                          "returnIndex " + toString(pbufs.returnIndex[i]) + " of first point in multiple return sequence not zero", &cv, blockStart + i);
          }
        }
        else
        {
          ...
          if (pbufs.returnCount[i] != prevReturnCount)
          {
            PRINT_MESSAGE(1000 /*???*/, proto.get("returnCount"),
                          "returnCount " + toString(pbufs.returnCount[i]) + " changed within a multiple return sequence", &cv, blockStart + i);
          }
          else
          {
            ...
            {
              PRINT_MESSAGE(1000 /*???*/, proto.get("returnIndex"), "returnIndex " + toString(pbufs.returnIndex[i]) + " is not incrementing by one", &cv,
                            blockStart + i);
            }
            else
            {
...
void E57Validator::validateImages2D(Node n)
{
  ...
  for (int64_t i = 0; i < v.childCount(); i++)
  {
    ...
    if (sn.isDefined("pinholeRepresentation") || sn.isDefined("sphericalRepresentation") || sn.isDefined("cylindricalRepresentation"))
...
void E57Validator::validateIntensityLimits(Node n)
{ // added by sc
  /// Validate type and downcast
  if (!validateStructure(n))
    return;
...
void E57Validator::validateColorLimits(Node n)
{ // added by sc
  /// Validate type and downcast
  if (!validateStructure(n))
    return;
