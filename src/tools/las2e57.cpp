...
: isFixedSize(isFixedSize_arg), areMembersContiguous(areMembersContiguous_arg), minimumId(minimumId_arg), maximumId(maximumId_arg), groups(0)
{
  if (isFixedSize)
  {
...
SparseGroupingScheme::SparseGroupingScheme(bool areMembersContiguous_arg) : areMembersContiguous(areMembersContiguous_arg) {}
...
void SparseGroupingScheme::write(ImageFile imf, StructureNode groupingSchemeNode, ustring idElementName, ustring /*description*/)
{
  ...
  sourceBuffers.push_back(
    SourceDestBuffer(imf, "cartesianBounds/xMinimum", &(groupVector[0].bbox.minimum[0]), groupVector.size(), false, false, sizeof(GroupRecord)));
  bbox.set("xMaximum", FloatNode(imf, 0.0, E57_DOUBLE));
  sourceBuffers.push_back(
    SourceDestBuffer(imf, "cartesianBounds/xMaximum", &(groupVector[0].bbox.maximum[0]), groupVector.size(), false, false, sizeof(GroupRecord)));
  bbox.set("yMinimum", FloatNode(imf, 0.0, E57_DOUBLE));
  sourceBuffers.push_back(
    SourceDestBuffer(imf, "cartesianBounds/yMinimum", &(groupVector[0].bbox.minimum[1]), groupVector.size(), false, false, sizeof(GroupRecord)));
  bbox.set("yMaximum", FloatNode(imf, 0.0, E57_DOUBLE));
  sourceBuffers.push_back(
    SourceDestBuffer(imf, "cartesianBounds/yMaximum", &(groupVector[0].bbox.maximum[1]), groupVector.size(), false, false, sizeof(GroupRecord)));
  bbox.set("zMinimum", FloatNode(imf, 0.0, E57_DOUBLE));
  sourceBuffers.push_back(
    SourceDestBuffer(imf, "cartesianBounds/zMinimum", &(groupVector[0].bbox.minimum[2]), groupVector.size(), false, false, sizeof(GroupRecord)));
  bbox.set("zMaximum", FloatNode(imf, 0.0, E57_DOUBLE));
  sourceBuffers.push_back(
    SourceDestBuffer(imf, "cartesianBounds/zMaximum", &(groupVector[0].bbox.maximum[2]), groupVector.size(), false, false, sizeof(GroupRecord)));
...
: useInfo_(useInfo), groupByLine_(0, useInfo.maximumColumnIndex, false, true), groupByReturnIndex_(0, useInfo.maximumReturnIndex),
  groupByScanDirectionFlag_(0, 1), groupByEdgeOfFlightLine_(0, 1), groupByClassification_(useInfo.minimumClassification, useInfo.maximumClassification),
  groupBySynthetic_(0, 1), groupByKeyPoint_(0, 1), groupByWithheld_(0, 1), groupByScanAngleRank_(useInfo.minimumScanAngleRank, useInfo.maximumScanAngleRank),
...
void GroupingSchemes::write(ImageFile imf)
{
  ...
  if (useInfo_.returnNumberUsed)
  {
    ...
    groupByReturnIndex_.write(imf, groupingSchemeNode, "returnIndex", "Points are grouped into return pulse positions. returnIndex=0 is first return.");
  }
  if (useInfo_.scanDirectionFlagUsed)
  {
    ...
    groupByScanDirectionFlag_.write(
      imf, groupingSchemeNode, "las:scanDirectionFlag",
  ...
  if (useInfo_.edgeOfFlightLineUsed)
  {
    ...
    groupByEdgeOfFlightLine_.write(imf, groupingSchemeNode, "las:edgeOfFlightLine", "Points are grouped edge and non-edge points in the scan lines.");
  }
  if (useInfo_.classificationUsed)
  {
  ...
  if (useInfo_.scanAngleRankUsed)
  {
    ...
    groupByScanAngleRank_.write(
      imf, groupingSchemeNode, "las:scanAngleRank",
  ...
  if (useInfo_.pointSourceIdUsed)
  {
    ...
    groupByPointSourceId_
      .write(imf, groupingSchemeNode, "las:pointSourceId",
...
int main(int argc, char** argv)
{
  ...
  tmpFlag |= _CRTDBG_ALLOC_MEM_DF | _CRTDBG_CHECK_ALWAYS_DF | _CRTDBG_CHECK_CRT_DF | _CRTDBG_LEAK_CHECK_DF;
...
void copyPerPointData(CommandLineOptions& options, LASReader& lasf, ImageFile imf, UseInfo& useInfo, GroupingSchemes& groupings, WaveformDatabase& /*waveDb*/)
{
  ...
  case 5:
    sourceBuffers.push_back(
      SourceDestBuffer(imf, "las:wavePacketDescriptorIndex", &pointBuffer[0].wavePacketDescriptorIndex, N, true, false, sizeof(LASPointDataRecord)));
    proto.set("las:wavePacketDescriptorIndex", IntegerNode(imf, 0, E57_UINT8_MIN, E57_UINT8_MAX));

    sourceBuffers.push_back(SourceDestBuffer(imf, "las:byteOffsetToWaveformData", reinterpret_cast<int64_t*>(&pointBuffer[0].byteOffsetToWaveformData), N, true,
                                             false, sizeof(LASPointDataRecord)));
    proto.set("las:byteOffsetToWaveformData", IntegerNode(imf, 0LL, 0LL, E57_INT64_MAX));

    sourceBuffers.push_back(
      SourceDestBuffer(imf, "las:waveformPacketSizeInBytes", &pointBuffer[0].waveformPacketSizeInBytes, N, true, false, sizeof(LASPointDataRecord)));
    proto.set("las:waveformPacketSizeInBytes", IntegerNode(imf, 0LU, E57_UINT32_MIN, E57_UINT32_MAX));

    sourceBuffers.push_back(
      SourceDestBuffer(imf, "las:returnPointWaveformLocation", &pointBuffer[0].returnPointWaveformLocation, N, true, false, sizeof(LASPointDataRecord)));
