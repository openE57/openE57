...
public:
  ...
  int32_t isAtomicClockReferenced; //!< This element should be present, and its value set to 1 if, and only if, the time stored in the dateTimeValue element is
                                   //!< obtained from an atomic clock time source. Shall be either 0 or 1.
  ...
  void SetSystemTime(SYSTEMTIME sysTim //!< Windows System Time
  );
  //! @brief This function gets the date and time from the Windows System Time;
  void GetSystemTime(SYSTEMTIME& sysTim //!< Windows System Time
  );
#endif
};
...
public:
  ...
  e57::CartesianBounds
    cartesianBounds; //!< The bounding box (in Cartesian coordinates) of all points in the group (in the local coordinate system of the points).
  e57::SphericalBounds
    sphericalBounds; //!< The bounding region (in spherical coordinates) of all the points in the group (in the local coordinate system of the points).
};
...
public:
  ...
  double pointRangeMinimum;       //!< Indicates that the PointRecord cartesian and range fields should be configured with this minimum value -E57_FLOAT_MAX or
  ...
  double angleMinimum; //!< Indicates that the PointRecord angle fields should be configured with this minimum value -E57_FLOAT_MAX or -E57_DOUBLE_MAX. If using
                       //!< a ScaledIntegerNode then this needs to be a minimum angle value.
  double angleMaximum; //!< Indicates that the PointRecord angle fields should be configured with this maximum value E57_FLOAT_MAX or E57_DOUBLE_MAX. If using a
                       //!< ScaledIntegerNode then this needs to be a maximum angle value.
  double angleScaledInteger; //!< Indicates that the PointRecord angle fields should be configured as a ScaledIntegerNode with this scale setting. If 0. then
                             //!< use FloatNode.
  ...
  double timeMaximum; //!< Indicates that the PointRecord timeStamp fields should be configured with this maximum value. like E57_UINT32_MAX, E57_FLOAT_MAX or
                      //!< E57_DOUBLE_MAX
  double timeMinimum; //!< Indicates that the PointRecord timeStamp fields should be configured with this minimum value -E57_FLOAT_MAX or -E57_DOUBLE_MAX. If
                      //!< using a ScaledIntegerNode then this needs to be a minimum time value.
  double timeScaledInteger; //!< Indicates that the PointRecord timeStamp fields should be configured as a ScaledIntegerNode with this scale setting. If 0. then
                            //!< use FloatNode, if -1. use IntegerNode.
  ...
  double intensityScaledInteger;  //!< Indicates that the PointRecord intensity fields should be configured as a ScaledIntegerNode with this setting. If 0. then
                                  //!< use FloatNode, if -1. use IntegerNode
...
/*! @details This structure is not actually used but is here for completeness.  The size and type of each element of the PointRecord can be configure in E57.
 * This is the configuration used by the SimpleAPI.
 */
...
public:
  ...
  bool cartesianInvalidState; //!< Indicates whether the Cartesian coordinate vector or its magnitude is meaningful. Value = 0 if the point is considered valid,
                              //!< 1 otherwise.  Shall be in the interval [0, 1].
  ...
  bool   sphericalInvalidState; //!< Indicates whether the spherical coordinate vector or its range value are meaningful. Value = 0 if the point is considered
                                //!< valid, 1 otherwise. Shall be in the interval [0, 1].
  ...
  uint32_t
    columnIndex; //!< The column number of point (zero based). This is useful for data that is stored in a regular grid. Shall be in the interval [0, 2^31)

  uint8_t returnIndex; //!< Only for multi-return sensors. The number of this return (zero based). That is, 0 is the first return, 1 is the second, and so on.
                       //!< Shall be in the interval [0, returnCount).
  uint8_t
    returnCount; //!< Only for multi-return sensors. The total number of returns for the pulse that this corresponds to. Shall be in the interval (0, 2^7).

  double timeStamp; //!< The time (in seconds) since the start time for the data, which is given by acquisitionStart in the parent Data3D Structure. Shall be
                    //!< non-negative
  bool isTimeStampInvalid; //!< Indicates whether the timeStamp element is meaningful. Value = 0 if the timestamp is considered valid, 1 otherwise. Shall be in
                           //!< the interval [0, 1].

  double intensity;        //!< Point response intensity. Unit is unspecified
  bool isIntensityInvalid; //!< Indicates whether the intensity element is meaningful. Value = 0 if the intensity is considered valid, 1 otherwise. Shall be in
                           //!< the interval [0, 1].
  ...
  bool     isColorInvalid; //!< Indicates whether the colorRed, colorBlue, and colorGreen elements are meaningful. Value = 0 if the color is considered valid, 1
                           //!< otherwise. Shall be in the interval [0, 1].
};
...
public:
  ...
  double  principalPointX; //!< The X coordinate in the image of the principal point, (in pixels). The principal point is the intersection of the z axis of the
                           //!< camera coordinate frame with the image plane.
...
public:
  ...
  double  radius; //!< The closest distance from the cylindrical image surface to the center of projection (that is, the radius of the cylinder) (in meters).
...
public:
  ...
  e57::VisualReferenceRepresentation visualReferenceRepresentation; //!< Representation for an image that does not define any camera projection model. The image
                                                                    //!< is to be used for visual reference only
...
public:
  ...
  Reader(const ustring& filePath //!< file path string
  );
  ...
  */
  bool GetImage2DSizes(int32_t                 imageIndex,      //!< This in the index into the image2D vector
...
public:
  ...
  Writer(const ustring& filePath,          //!< file path string
  ...
  int32_t NewImage2D(Image2D& image2DHeader //!< pointer to the Image2D structure to receive the picture information
  ) const;                                  //!< @return Returns the image2D index

  //! @brief This function writes the image block of data
  int64_t WriteImage2DData(int32_t                imageIndex,      //!< picture block index given by the NewImage2D
                           e57::Image2DType       imageType,       //!< identifies the image format desired.
                           e57::Image2DProjection imageProjection, //!< identifies the projection desired.
