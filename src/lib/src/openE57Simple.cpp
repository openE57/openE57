...
This browser-based document describes the E57 Simple API (Application Programmer Interface) version 0.52, which is a collection of functions that help a C++
programmer wrap the E57 Foundation API.
...
void DateTime::SetUTCDateTime(int   utc_year,   //!< The year 1900-9999
...
{
  dateTimeValue = e57::GetGPSDateTimeFromUTC(utc_year, utc_month, utc_day, utc_hour, utc_minute, utc_seconds);

  isAtomicClockReferenced = 0;
}

void DateTime::GetUTCDateTime(int&   utc_year,   //!< The year 1900-9999
                              int&   utc_month,  //!< The month 0-11
                              int&   utc_day,    //!< The day 1-31
                              int&   utc_hour,   //!< The hour 0-23
                              int&   utc_minute, //!< The minute 0-59
...
{
  e57::GetUTCFromGPSDateTime(dateTimeValue, utc_year, utc_month, utc_day, utc_hour, utc_minute, utc_seconds);
}
#if defined(WIN32)

void DateTime::SetSystemTime(SYSTEMTIME sysTim //!< Windows System Time
)
{
...
void DateTime::GetSystemTime(SYSTEMTIME& sysTim //!< Windows System Time
)
{
...
bool Reader ::ReadImage2D(int32_t imageIndex, Image2D& image2DHeader) const
{
  return impl_->ReadImage2D(imageIndex, image2DHeader);
}

bool Reader ::GetImage2DSizes(int32_t                 imageIndex,      // This in the index into the image2D vector
...
{
  return impl_->GetImage2DSizes(imageIndex, imageProjection, imageType, imageWidth, imageHeight, imageSize, imageMaskType, imageVisualType);
}

int64_t Reader ::ReadImage2DData(int32_t                imageIndex,      // picture block index
                                 e57::Image2DProjection imageProjection, // identifies the projection desired.
                                 e57::Image2DType       imageType,       // identifies the image format desired.
...
bool Reader ::GetData3DSizes(int32_t  dataIndex,   // This in the index into the images3D vector
...
bool Reader ::ReadData3DGroupsData(int32_t  dataIndex,       // data block index given by the NewData3D
...
) const
{
  return impl_->SetUpData3DPointsData(dataIndex, pointCount, cartesianX, cartesianY, cartesianZ, cartesianInvalidState, intensity, isIntensityInvalid, colorRed,
                                      ...
                                      rowIndex, columnIndex, returnIndex, returnCount, timeStamp, isTimeStampInvalid, pointDataExtension);
}
...
//	e57::Writer
//
Writer ::Writer(const ustring& filePath,          // file path string
...
int32_t Writer ::NewImage2D(Image2D& image2DHeader // pointer to the Image2D structure to receive the picture information
) const                                            // /return Returns the image2D index
{
  return impl_->NewImage2D(image2DHeader);
}

int64_t Writer ::WriteImage2DData(int32_t                imageIndex,      // picture block index given by the NewImage2D
                                  e57::Image2DType       imageType,       // identifies the image format desired.
                                  e57::Image2DProjection imageProjection, // identifies the projection desired.
                                  void*                  pBuffer,         // pointer the buffer
...
int32_t Writer ::NewData3D(Data3D& data3DHeader,                                      // pointer to the Data3D structure to receive the image information
...
bool Writer ::WriteData3DGroupsData(int32_t  dataIndex,       // data block index given by the NewData3D
                                    int32_t  groupCount,      //!< size of each of the buffers given
                                    int64_t* idElementValue,  // index for this group
                                    int64_t* startPointIndex, // Starting index in to the "points" data vector for the groups
                                    int64_t* pointCount       // size of the groups given
