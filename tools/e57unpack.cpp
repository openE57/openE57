...
void print_version()
{
  cout << PROGRAM_NAME " (" << BOOST_PLATFORM << ") " << PROGRAM_VERSION << "." << '\n';
...
void print_help(const options_description& options)
{
  cout << "Usage:\n"
...
class get_at : public static_visitor<variant<double, int64_t, ustring>>
{
  size_t at;
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    options.add_options()("version,v", "show version")("help,h", "show help")("format,F", value<std::string>(),
      ...
      "  (--dst may be omitted)")("no-images", "supress image output")("no-points", "supress pointcloud output");

    positional_options_description positional;
    positional.add("src", 1);
    positional.add("dst", 2);

    store(command_line_parser(argc, argv).options(options).positional(positional).run(), opt);
    ...
    if (opt.count("src"))
    {
      ...
      if (root.isDefined("data3D") && !opt.count("no-points"))
      {
        ...
        for (int64_t child = 0; child < data3D.childCount(); ++child)
        {
          ...
          ofstream inf(dst / path(string("image3d-") + lexical_cast<string>(child) + ".inf"));
